const Rules = function(board) {
  ...
  this.getRandomValidMove = function() {
    ...
    for (let row = 0; row < board.boardSize; row++) {
      for (let col = 0; col < board.boardSize; col++) {
        ...
        for (i = 0; i < 4; i++) {
          ...
          const numGemsCrushed = this.numberGemsCrushedByMove(fromGem, direction);
          if (numGemsCrushed == 3) {
    ...
    const searchArray = validMovesThreeCrush.length ? validMovesThreeCrush :
  ...
  this.createSpecifiedBoard = function(boardSpec) {
    const color_dict = {'r':'red', 'o':'orange', 'y':'yellow', 'g':'green','b':'blue','p':'purple'}
    let numChars = 0;
